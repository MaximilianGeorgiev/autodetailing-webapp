...
export const CreatePromotion = () => {
    ...
    const [inputValues, setInputValues] = useState({
        dateFrom: { value: new Date(), error: false, errorMsg: "" },
        dateTo: { value: new Date, error: false, errorMsg: "" },
    ...
    const populateProductDropDown = () => {
        ...
        for (const product of products)
            menuItems.push(<MenuItem value={product.product_id}>{product.product_title}</MenuItem>);
    ...
    const populateServiceDropDown = () => {
        ...
        for (const service of services)
            menuItems.push(<MenuItem value={service.service_id}>{service.service_title}</MenuItem>);
    ...
    const processPromotionCreationForm = () => {
        ...
        if (!inputValues?.dateFrom?.value) updatedState.dateFrom = {
            value: updatedState.dateFrom.value,
            error: true,
            errorMsg: "Starting date cannot be empty.",
        };

        if (!inputValues?.dateTo?.value) updatedState.dateTo = {
            value: updatedState.dateTo.value,
            error: true,
            errorMsg: "Ending date cannot be empty.",
        };

        if (!inputValues?.newPrice?.value) updatedState.newPrice = {
            value: updatedState.newPrice.value,
            error: true,
            errorMsg: "Price field cannot be empty.",
        };

        if (!inputValues?.selectedEntity?.value) updatedState.selectedEntity = {
        ...
        if (inputValues?.title?.value
            ...
            createPromotion(inputValues.title.value, inputValues.description.value, inputValues.newPrice.value, inputValues.selectedEntity.value).then((res) => {
                navigate("/");
            })
        }
                ...
                >
                    ...
                    <Box noValidate sx={{ mt: 1 }}>
                        ...
                        <FormControl
                            ...
                            margin="normal"
                        >
                            <InputLabel id="demo-simple-select-label">{categoryToggle ? "Product" : "Service"}</InputLabel>
                            <Select
                            ...
                            >
                                {categoryToggle ? populateProductDropDown() : populateServiceDropDown()}
                            </Select>
                        </FormControl>
                        <Button
                            type="submit"
